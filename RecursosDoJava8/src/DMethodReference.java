...
// e essa inclusive eh a maneira de se ler uma funcao lambda.
...
// mas a funcao lambda funciona mesmo assim. O mesmo vale para method reference.
...
public class DMethodReference {
	public static void main(String[] args) {
		...
		// A funcao lambda do passo 1 do codigo C pode ser convertida, pois dado 1 argumento (String s), ela invoca o metodo println()
		...
		// Esta funcao lambda NAO PODE ser convertida para method reference, pois recebe dois argumentos (s1 e s2)
		...
		// A funcao lambda do passo 3 do programa C tambem NAO PODE ser convertida para method reference, pois ela recebe dois argumentos (s1 e s2).
		...
		// uma Interface Funcional chamada Function, portanto, podemos substitui-la pela funcao lambda abaixo
		...
		// transforma a funcao lambda em um comparator, utilizando o metodo que a funcao lambda chama como criterio de comparacao.
		
		// Voce reparou no que fizemos acima? Com a ajuda do metodo default abstrato Comparator.comparing(), transformamos uma funcao lambda que antes usava dois argumentos
		// (s1 e s2) em uma funcao lambda que agora recebe apenas um argumento (String s) e, dado este argumento, chama o metodo length do proprio argumento.
		// Se a funcao lambda recebe um argumento e chama um metodo, podemos utilizar o method reference em seu lugar. Ficaria assim:
